...
//Old readQuote from premade string
/*function readQuote(day) {
    ...
    .get()   //the collection called "hikes"
        .then(allHikes=> {
            ...
            allHikes.forEach(doc => { //iterate thru each doc
                ...
                //newcard.querySelector('.card-length').innerHTML = hikeLength +"km"; //commented out cause the line above also use .cardlength
                newcard.querySelector('.card-text').innerHTML = details;
                newcard.querySelector('.card-image').src = `./images/${hikeCode}.jpg`; //Example: NV01.jpg
                newcard.querySelector('a').href = "eachHike.html?docID="+docID;

                //Th method below only works with one i icon in the html code (the one line bookmark) not multiple. 
                newcard.querySelector('i').id = 'save-' + docID;   //guaranteed to be unique  This is for the bookmark icon
                // this line will call a function to save the hikes to the user's document
                newcard.querySelector('i').onclick = () => saveBookmark(docID);  //EventListener for the bookmark icon

                //Ensure that the bookmark displays correctly as filled if it is already in the favourites
                //go to the users document and check if the bookmarks array already includes thhe hikes docID
                //if so display the solid bookmark
                currentUser.get().then(userDoc => {
                    //get the user name
...
//OLD insertName replaced with the one above
/* function insertNameFromFirestore() {
